...
begin
  ...
  select *
  into v_user
  from public.tts_usage_daily
  where usage_date = v_usage_date and scope = 'user' and scope_id = p_user_id
  for update;

  select *
  into v_guild
  from public.tts_usage_daily
  where usage_date = v_usage_date and scope = 'guild' and scope_id = p_guild_id
  for update;

  select *
  into v_global
  from public.tts_usage_daily
  where usage_date = v_usage_date and scope = 'global' and scope_id = 'global'
  for update;
